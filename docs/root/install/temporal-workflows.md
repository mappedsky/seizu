...
| `REMEDIATION_AGENT_BASE_URL` | `""` | LLM gateway/proxy base URL exported to the agent phase (`ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL`); typically paired with the key command so the sandbox only ever holds a short-lived gateway key. Mutually exclusive with the credential proxy below. |
| `REMEDIATION_CREDENTIAL_PROXY_ENABLED` | `false` | Run a short-lived LiteLLM proxy in its **own** sandbox holding the real provider key, and hand the agent sandbox only a budget-capped virtual key pointed at it (see below). `claude`/`codex` only. |
| `REMEDIATION_CREDENTIAL_PROXY_MAX_BUDGET` | `5` | USD spend cap on the per-run virtual key — bounds real-time abuse of a key stolen while the proxy is up. |
