...
| `SANDBOX_AGENT_MODEL` | `""` | Model for the CLI. For `claude`/`codex` a bare model override (empty → the CLI's default). For `opencode` it is **required** and takes the `provider/model` form (e.g. `deepseek/deepseek-chat`), which also selects the provider key and — in credential-proxy mode — the LiteLLM namespace. |
