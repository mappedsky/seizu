...
To smoke-test the sandbox side directly (auth, gh, git) without running the full workflow, create an E2B sandbox and reproduce the phases with the token passed as a **per-command** env (matching the credential isolation), e.g. via `e2b_code_interpreter.AsyncSandbox`: `commands.run("gh auth setup-git --hostname github.com && git clone … && git commit --allow-empty && git push", envs={"GH_TOKEN": token, "GH_HOST": "github.com"})`. A successful push (then delete the test branch) confirms the auth path end-to-end. Check the token's write access first with `gh api repos/<org>/<repo> --jq .permissions` — `push` must be `true`, and a fine-grained PAT needs Contents + Pull requests read/write, org approval, and an owner who has write access to the repo.
