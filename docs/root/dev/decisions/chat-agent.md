...
seed matches stored profiles by exact name, the same value protected by the
API's uniqueness constraint. Export preserves an existing local key by name and
otherwise derives one. Reconciliation processes the declared default before
every non-default profile, and profiles precede the rest of the seed pipeline.

**Why:** profile ids are generated by the server, so exporting them would make a
configuration deployment-specific. Changing away from the current default
first is rejected by the store's exactly-one-enabled-default invariant, while
