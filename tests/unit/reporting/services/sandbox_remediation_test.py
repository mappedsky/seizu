...
def _phase_of(cmd: str) -> str:
    ...
    if "litellm --config" in cmd:
        return "proxy_start"
    # The install reads the hash-locked file; it never names a requirement.
...
async def test_credential_proxy_installs_the_hash_locked_set_without_a_template() -> None:
    ...
    assert "litellm==1.96.0" in lock and "--hash=sha256:" in lock
    # pip reads the file; the env only carries the runtime the lock was
    # resolved for, which the script checks the sandbox against.
    assert set(dict(proxy.calls)["proxy_install"]) == {"SEIZU_LOCK_PYTHON", "SEIZU_LOCK_MACHINE"}
    assert result.status == "completed"


async def test_run_refuses_before_any_sandbox_when_the_lock_is_unusable() -> None:
    # A proxy run creates sandboxes and hands one the real provider key, so a
    # missing lock must stop it before any of that — not at install time.
    opens: list[dict[str, Any]] = []
    with (
        _settings(
            SANDBOX_AGENT_CREDENTIAL_PROXY_ENABLED=True,
    ...
    assert "lock" in (result.error or "")
    assert opens == []
...
def test_proxy_install_script_verifies_the_import() -> None:
    script = sandbox_agent._PROXY_LOCKED_INSTALL
    # No literal requirement in the script (it comes from the lock file), and no
    # "skip if litellm already exists" short-circuit — an unrelated litellm in
    # the image says nothing about whether its proxy imports.
    ...
    assert "pip install --no-deps --require-hashes -r" in script
