...
def _phase_of(cmd: str) -> str:
    ...
    if "litellm --config" in cmd:
        return "proxy_start"
    # Either install plan: the hash-locked file (default) or the top-level pins
    # passed as an env var. Neither names a literal requirement in the script.
    if "--require-hashes" in cmd or "SEIZU_PROXY_REQUIREMENTS" in cmd:
...
async def test_credential_proxy_installs_the_hash_locked_set_by_default() -> None:
    ...
    lock = proxy.files[sandbox_agent._PROXY_LOCK_SANDBOX_PATH]
    assert lock.startswith("# Generated by `make lock_proxy_requirements`")
    assert "litellm==1.87.0" in lock and "--hash=sha256:" in lock
    # No requirement env var on the locked path — pip reads the file.
    assert dict(proxy.calls)["proxy_install"] == {}
    assert result.status == "completed"


async def test_credential_proxy_falls_back_to_top_level_pins_when_unlocked() -> None:
    # An operator pinning something the lock does not cover still gets an
    # exactly-pinned install, just without the transitive hashes.
    proxy = _FakeBackend()
    agent = _FakeBackend()
    push = _FakeBackend(outputs={"push": "SEIZU_PR_URL=https://github.com/org/app/pull/1\n"})
    with (
...
def test_proxy_install_scripts_pin_and_verify_the_import() -> None:
    locked = sandbox_agent._PROXY_LOCKED_INSTALL
    pinned = sandbox_agent._PROXY_PINNED_INSTALL
    for script in (locked, pinned):
        # No literal requirement in either script (they come from the lock file
        # or the env var), and no "skip if litellm already exists" short-circuit
        # — an unrelated litellm in the image says nothing about its proxy.
        assert "litellm[proxy]" not in script
        assert "command -v litellm" not in script
        # The import is what actually broke; check it here rather than letting
        # the CLI die at boot behind a health-check timeout.
        assert "import litellm.proxy.proxy_server" in script
    # --no-deps: the lock is the whole closure, and the sandbox image's pip
    # rejects the install outright if asked to re-derive dependencies.
    assert "pip install --no-deps --require-hashes -r" in locked
    assert "pip install --quiet $SEIZU_PROXY_REQUIREMENTS" in pinned


def test_checked_in_lock_matches_the_default_requirements() -> None:
    # The lock only applies while it matches the configured requirements, so a
    # pin bumped without `make lock_proxy_requirements` silently drops back to
    # top-level-only pinning. Catch that here instead of in production.
    with _settings():
        assert sandbox_agent.proxy_lock_input() == sandbox_agent.proxy_requirements()
        assert sandbox_agent.proxy_install_plan().locked is True
...
async def test_sandbox_creation_failure_names_the_sandbox_not_the_last_command() -> None:
    # A bad template (or any create failure) fails between commands. Blaming it
    # on whatever ran last in the *previous* sandbox is worse than useless.
    agent = _FakeBackend()

    @asynccontextmanager
    async def _ctx(**kwargs: Any):  # type: ignore[misc]
        if kwargs.get("template") is None:  # the push sandbox
            raise RuntimeError("template not found")
        yield agent

    with _settings(), _patch_open_backend(_ctx):
        result = await run_remediation(**_TARGET)

    assert result.status == "failed"
    assert (result.error or "").startswith("push_sandbox phase: ")


async def test_proxy_sandbox_creation_failure_is_attributed_to_the_proxy() -> None:
    # The likeliest failure of the new template setting: it names a template
    # that does not exist, so the proxy sandbox never gets created.
    @asynccontextmanager
    async def _ctx(**kwargs: Any):  # type: ignore[misc]
        raise RuntimeError("template 'seizu-litellm-proxy' not found")
    ...
    with (
        ...
        _patch_open_backend(_ctx),
    ):
        result = await run_remediation(**_TARGET)

    assert result.status == "failed"
    assert (result.error or "").startswith("proxy_sandbox phase: ")


async def test_command_timeout_reports_its_own_bound_not_the_run_deadline() -> None:
    # The proxy phases carry fixed bounds far below REMEDIATION_TIMEOUT_SECONDS,
    # so reporting the run-wide deadline would name a duration that never
    # elapsed.
    proxy = _FakeBackend()

    async def _timeout(cmd: str, *, timeout_seconds: int, on_output: Any, envs: Any = None) -> str:
        raise TimeoutError()

    proxy.run_bash_streaming = _timeout  # type: ignore[method-assign]
    with (
        _settings(
            SANDBOX_AGENT_CREDENTIAL_PROXY_ENABLED=True,
            SANDBOX_AGENT_API_KEY="real-anthropic-key",
    ...
    assert error.startswith("proxy_install phase: ")
    assert f"{sandbox_agent._PROXY_INSTALL_TIMEOUT_SECONDS}s" in error
    assert "1800s" not in error
