...
- The credential proxy (:func:`credential_proxy`) runs a short-lived LiteLLM
  ...
  exactly as built and never inspected, or — with no template — by installing a
  **hash-locked requirement file** (``sandbox_proxy_requirements.txt`` by
  default, regenerated by ``make lock_proxy_requirements``) at run time. An
  unpinned install broke the proxy in the field when a transitive release
  dropped a symbol LiteLLM's proxy imports, and this VM holds the real provider
  key. The agent-CLI-to-proxy wire remains unverified against a live CLI —
  confirm with ``make remediation_smoke SMOKE_PROXY=1`` before relying on the
  proxy path.
"""
...
#
# - *Template* (SANDBOX_AGENT_CREDENTIAL_PROXY_TEMPLATE): an image the operator
#   supplied — ours (scripts/build_proxy_template.py) or entirely their own. Its
#   only contract is that it can run a LiteLLM proxy. The run uses it as it is:
#   no install, and no inspection of what it contains, because the checked-in
#   lock is one valid answer rather than the only one (it drifts from upstream
#   by design, and an operator may want a newer LiteLLM than we have pinned).
#   Keeping a template current is entirely theirs to manage.
# - *No template*: install the hash-locked requirement file into the base image.
#
# When the install does run, two rules hold, both learned the hard way:
#
...
# The requirement set is a hash-locked file, never a requirement string: every
# transitive package is pinned with hashes, so the sandbox that holds the real
# provider key runs a fixed set of artifacts instead of whatever resolves that
# day. (Pinning only the top-level requirement is not enough — LiteLLM pins
# FastAPI exactly but leaves pydantic/aiohttp/openai/httpx on ranges.)
...
# A lock is only valid for the interpreter and architecture it was resolved for:
# its hashes cover wheels built for that ABI. Check before installing, because
# the alternative is a "no matching distribution" wall of text (or, worse, an
# install that half-succeeds) in a sandbox nobody is watching. Cheap: same
# command, no extra round trip.
_PROXY_LOCKED_INSTALL = f"""\
set -euo pipefail
sandbox_python="$(python3 -c 'import sys; print("%d.%d" % sys.version_info[:2])')"
sandbox_machine="$(uname -m)"
if [ "$sandbox_python" != "$SEIZU_LOCK_PYTHON" ] || [ "$sandbox_machine" != "$SEIZU_LOCK_MACHINE" ]; then
  echo "SEIZU_PROXY_RUNTIME_MISMATCH: this sandbox is python $sandbox_python on $sandbox_machine, but the"
  echo "requirement lock was resolved for python $SEIZU_LOCK_PYTHON on $SEIZU_LOCK_MACHINE. Re-lock for this"
  echo "runtime (make lock_proxy_requirements PYTHON_VERSION=$sandbox_python OUTPUT=<path>, then point"
  echo "SANDBOX_AGENT_CREDENTIAL_PROXY_REQUIREMENTS_FILE at it) or use a prebuilt template."
  exit 1
fi
...
"""
# Env the check above reads (non-secret), and the lock header markers carrying it.
_PROXY_LOCK_PYTHON_ENV = "SEIZU_LOCK_PYTHON"
_PROXY_LOCK_MACHINE_ENV = "SEIZU_LOCK_MACHINE"
# The checked-in lock, used when SANDBOX_AGENT_CREDENTIAL_PROXY_REQUIREMENTS_FILE
# is empty. Its header records what it was compiled from and for.
DEFAULT_PROXY_LOCK_PATH = str(Path(__file__).with_name("sandbox_proxy_requirements.txt"))
PROXY_LOCK_INPUT_MARKER = "# seizu-proxy-input:"
PROXY_LOCK_RUNTIME_MARKER = "# seizu-proxy-runtime:"
...
def agent_config_error() -> str | None:
    ...
    if settings.SANDBOX_AGENT_CREDENTIAL_PROXY_ENABLED:
        ...
        # The lock is what a templateless run installs; with a template the
        # image is used as built, so it is irrelevant.
        if not settings.SANDBOX_AGENT_CREDENTIAL_PROXY_TEMPLATE and (err := proxy_lock_error()) is not None:
            return err
...
class ProxyInstallPlan:
    """How a templateless proxy sandbox is provisioned: the script, its env, and
    the files written into the sandbox first."""
...
def proxy_lock_path() -> str:
    """The configured lock file, or the checked-in one."""
    from reporting import settings
...
def proxy_lock_error() -> str | None:
    """Return why the configured lock cannot be used, or None.

    Checked before a run starts rather than at install time: a proxy run creates
...
def proxy_install_plan() -> ProxyInstallPlan | None:
    """How to provision a proxy sandbox that has no template, or None when the
    lock is unusable (see :func:`proxy_lock_error`).

    Also the recipe `scripts/build_proxy_template.py` bakes into a template, so
    a template and a templateless run install the same set.
    """
    ...
    return ProxyInstallPlan(
        ...
        files={_PROXY_LOCK_SANDBOX_PATH: lock.text},
        lock=lock,
    )
...
async def credential_proxy(
    *,
    ...
    # Re-checked here, not just in agent_config_error(), so direct callers (e.g.
    # the smoke script) fail before a sandbox is created rather than mid-run.
    # Only matters without a template — with one nothing is installed.
    if not settings.SANDBOX_AGENT_CREDENTIAL_PROXY_TEMPLATE and (invalid := proxy_lock_error()) is not None:
    ...
    async with open_backend(
        ...
        if template is None:
            ...
            if plan is None:  # unreachable: agent_config_error() already checked
                raise RuntimeError(proxy_lock_error() or "the credential proxy requirement lock is unusable")
