...
Triggers mirror scheduled queries: a fixed frequency in minutes, or
``watch_scans`` against Cartography SyncMetadata updates.
"""
...
async def _is_triggered(item: ScheduledChatItem) -> bool:
    if item.frequency and _frequency_triggered(item.last_scheduled_at, item.frequency):
        logger.debug("Frequency trigger fired", extra={"scheduled_chat_id": item.scheduled_chat_id})
        return True
    if item.watch_scans:
